...
\title{CUDA Kernel Library\\[4pt]
\large Driving a GEMM from naive to compute bound on an RTX 5070}
...
\caption{GEMM ladder at 4096 cubed. Bar length is the absolute achieved throughput
(GFLOP/s). The percent in each label is of cuBLAS \emph{at the same precision},
which is why a shorter FP32 bar can show a higher percent than a longer tensor bar:
the FP32 kernels (blue) are measured against cuBLAS SGEMM (about 23 TFLOP/s) and the
tensor kernels (amber, top kernel green) against cuBLAS FP16 or BF16 (about 65
TFLOP/s).}
\label{fig:ladder}
\end{figure}
